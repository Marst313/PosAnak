...
import { deleteDataAnak, getDataAnak, getSingleDataAnak, setEditAnak, setGraph, setTambahDataAnak } from '../../features/kids/kids';
import { editAnakIcon, deleteIcon, graphAnakIcon } from '../../images/dataanak/';
...
const SingleDataAnak = ({ child, index }) => {
  ...
  return (
    <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-lightGreen/30 ">
      ...
      <td className="px-6 py-4 whitespace-nowrap capitalize">{child.nama}</td>
      <td className="px-6 py-4 ">{formattedDate}</td>
      <td className="px-6 py-4 whitespace-nowrap w-fit">{usia}</td>
      <td className="px-6 py-4 whitespace-nowrap w-fit capitalize">{child.namaIbu} </td>
      <td className="  py-4 flex gap-2 px-4  items-center justify-center ">
        <button type="button" onClick={openModal}>
          <img src={deleteIcon} alt="logo delete" />
        </button>
        <button type="button">
          <img src={editAnakIcon} alt="logo edit" onClick={() => handleEdit(child._id)} />
        </button>
      ...
      <th className={`overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full ${popUp ? 'flex' : 'hidden'}`}>
